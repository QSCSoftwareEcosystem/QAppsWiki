...
def to_json(graph, generated: str = "") -> dict:
    ...
    for nid, a in graph.nodes(data=True):
        nodes.append({"id": nid, **{k: v for k, v in a.items()}})
    edges = [
...
<body>
<div id="bar"><b>QAppsWiki</b> — {stats['nodes']} nodes, {stats['edges']} edges
  <span class="hint">&nbsp;|&nbsp; {legend} &nbsp;|&nbsp; red node = missing page, dashed red edge = AMBIGUOUS</span>
</div>
<div id="cy"></div>
<script>
